...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    ...
    this->children_.insert({key_char, std::move(child)});
...
class Trie {
 ...
 public:
  ...
  Trie() {
    ...
    this->root_ = std::make_unique<TrieNode>(std::move(root));
  }
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (const char ch : key) {
      if (!node->get()->HasChild(ch)) {
        node->get()->InsertChildNode(ch, std::make_unique<TrieNode>(TrieNode(ch)));
      }
    ...
    // node maybe nullptr
    if (node == nullptr) {
      parent->get()->InsertChildNode(key.back(), std::make_unique<TrieNodeWithValue<T>>(key.back(), value));
      this->latch_.WUnlock();
      return true;
    }
    ...
    // now node is TrieNode
    // we need turn TrieNode to TrieNodeWithValue
    auto new_node = std::move(*node);
    parent->get()->RemoveChildNode(key.back());
    parent->get()->InsertChildNode(key.back(), std::make_unique<TrieNodeWithValue<T>>(std::move(*new_node), value));
  ...
  bool Remove(const std::string &key) {
    ...
    this->latch_.WLock();
    auto node = &this->root_;
    RemoveInner(0, key, node, &ret);
    this->latch_.WUnlock();
    return ret;
  }

  bool RemoveInner(size_t idx, const std::string &key, std::unique_ptr<TrieNode> *parent, bool *ret) {
    if (idx == key.size()) {
      *ret = true;
      parent->get()->SetEndNode(false);
      return !parent->get()->HasChildren() && !parent->get()->IsEndNode();
    }

    if (!parent->get()->HasChild(key[idx])) {
      *ret = false;
      return false;
    }

    bool need_remove = RemoveInner(idx + 1, key, parent->get()->GetChildNode(key[idx]), ret);
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    if (!node->get()->IsEndNode() || (node->get()->GetKeyChar() != key.back())) {
      *success = false;
      this->latch_.RUnlock();
      return {};
    }
